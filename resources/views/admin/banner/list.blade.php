                ...
                <div class="title_right">
                    <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right">
                        @if (Helper::authorizing('Banner', 'Restore')['status'] == 'true')
                            <a href="{{ route('admin.banner.deleted') }}" class="btn btn-round btn-danger"
                                style="float: right; margin-bottom: 5px;" data-toggle="tooltip"
                                title="{{ ucwords(lang('view deleted items', $translation)) }}">
                                <i class="fa fa-trash"></i>
                            </a>
                        @endif
                        <a href="{{ route('admin.banner.create') }}" class="btn btn-round btn-success"
                            style="float: right;">
                            ...
                            if (response.data == '') {
                                ...
                                $.each(response.data, function(index, value) {
                                    html += '<tr role="row" id="row-' + value.id +
                                        '" title="{{ ucfirst(lang('Drag & drop to sorting', $translation)) }}" data-toggle="tooltip">';
                                    html += '<td class="dragndrop"><img src="' + value.image_item +
                                    ...
                                    var editUrl = '{{ route('admin.banner.edit', 0) }}';
                                    editUrl = editUrl.replace('/0', '/' + value.id);
                                    action_edit = '<a href="' + editUrl +
                                        '" class="btn btn-xs btn-primary" title="{{ ucwords(lang('edit', $translation)) }}"><i class="fa fa-pencil"></i>&nbsp; {{ ucwords(lang('edit', $translation)) }}</a>';
                                    action_delete =
                                        '<form action="{{ route('admin.banner.delete') }}" method="POST" onsubmit="return confirm(\'{{ lang('Are you sure to delete this #item?', $translation, ['#item' => $this_object]) }}\');" style="display: inline">{{ csrf_field() }}<input type="hidden" name="id" value="' +
                                        value.id +
                                        '"><button type="submit" class="btn btn-xs btn-danger" title="{{ ucwords(lang('delete', $translation)) }}"><i class="fa fa-trash"></i>&nbsp; {{ ucwords(lang('delete', $translation)) }}</button></form>';
                    ...
                    if (typeof response.status != 'undefined') {
                        if (response.status == 'true') {
                            ...
                            if (response.data == '') {
                                ...
                                $.each(response.data, function(index, value) {
                                    html += '<tr>';
                                    html += '<td><img src="' + value.image_item +
                                        '" style="max-width:200px;"></td>';
                                    html += '<td>' + value.title + '</td>';
                                    ...
                                    action_restore =
                                        '<form action="{{ route('admin.banner.restore') }}" method="POST" onsubmit="return confirm(\'{{ lang('Are you sure to restore this #item?', $translation, ['#item' => $this_object]) }}\');" style="display: inline">{{ csrf_field() }}<input type="hidden" name="id" value="' +
                                        value.id +
                                        '"><button type="submit" class="btn btn-xs btn-primary" title="{{ ucwords(lang('restore', $translation)) }}"><i class="fa fa-check"></i>&nbsp; {{ ucwords(lang('restore', $translation)) }}</button></form>';
